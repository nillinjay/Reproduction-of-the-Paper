...
import os
import time
from method import propagation_ARSS_sfft,phase_generation_4
...
class SGD(nn.Module):
    def __init__(self, phaseh, phaseu, phasev,phaseh2, feature_size, wavelength, prop_dist, num_iters, propagator=None,
    ...
    def forward(self, target_amp, init_phase=None):
        ...
        """
        final_phase = stochastic_gradient_descent(init_phase, target_amp, self.phaseh, self.phaseu, self.phasev,self.phaseh2, self.num_iters,
...
def stochastic_gradient_descent(init_phase, target_amp, phaseh, phaseu, phasev,phaseh2, num_iters, feature_size, wavelength, prop_dist, propagator=None,
    ...
    for k in range(num_iters):
        ...
        recon_field = propagator(u_in=slm_field_pad, phaseh=phaseh, phaseu=phaseu, phasev=phasev,phaseh2=phaseh2)
...
if __name__ == "__main__":
    ...
    # training staring
    sgd = SGD(phaseh=phaseh, phaseu=phaseu, phasev=phasev,phaseh2=phaseh2 ,feature_size=feature_size, wavelength=wavelength, prop_dist=prop_dist,num_iters=num_iters, propagator=propagator, loss=loss, lr=lr, lr_s=lr_s, s0=s0, device=device)
